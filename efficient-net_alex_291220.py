...
# -*- coding: utf-8 -*-
"""
Created on Tue Dec 29 15:13:57 2020
...
def load_images(path):
    ...
    for filename in os.listdir(path):
        ...
        if img is not None:
            ...
            curr = tensorflow.keras.applications.efficientnet.preprocess_input(curr, data_format=None)
...
#Main method calls functions for loading the pictures from given path, preprocesses them for Efficient Net and returns the file names as well as the Classification results as a list
def main (path):
    image_list, name_list = load_images(path)
    results = []
    for i in range(len(image_list)):
        y = model.predict(image_list[i])
        pred = tensorflow.keras.applications.efficientnet.decode_predictions(y, top = 5)
        results.append(pred)
